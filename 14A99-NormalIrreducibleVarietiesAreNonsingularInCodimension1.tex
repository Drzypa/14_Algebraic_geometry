...
\usepackage{pmmeta}
\pmcanonicalname{NormalIrreducibleVarietiesAreNonsingularInCodimension1}
\pmcreated{2013-03-22 13:20:20}
\pmmodified{2013-03-22 13:20:20}
\pmowner{archibal}{4430}
\pmmodifier{archibal}{4430}
\pmtitle{normal irreducible varieties are nonsingular in codimension 1}
\pmrecord{7}{33853}
\pmprivacy{1}
...
\newcommand{\Zp}{\Z/p}
\begin{document}
\begin{thm} Let $X$ be a normal irreducible variety.  The singular set $S\subset X$ has codimension 2 or more.
\end{thm}
\begin{proof}
Assume not.  We may assume $X$ is affine, since codimension is local.  Now let $\fr u$ be the ideal of functions vanishing on $S$.  This is an ideal of height 1, so the local ring of $Y$, $\O_S=A(X)_{\fr u}$, where $A(X)$ is the affine ring of $X$, is a 1-dimensional local ring, and integrally closed, since $X$ is normal. Any integrally closed 1-dimensional local domain is a 
